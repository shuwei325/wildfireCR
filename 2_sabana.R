...
sabana_st <-st_read('../datos/Area_sabana_2013.shp') 
...
modPO <- gamlss(INC ~ DAH+logFA+LST+logLSF+SLOPE+logTRI+TWI+WE+WExpo+sabana, 
                data = datos_training, 
                #newdata=datos_validation, 
                family="ZIP")

modZIP <- gamlss(INC ~ DAH+logFA+LST+logLSF+SLOPE+logTRI+TWI+WE+WExpo+sabana, 
                 #sigma.formula = ~DAH+logFA+LST+logLSF+SLOPE+logTRI+TWI+WE+WExpo,
                 data = datos_training, 
                 #newdata=datos_validation, 
                 family="ZIP")

modNBII <- gamlss(INC ~ DAH+logFA+LST+logLSF+SLOPE+logTRI+TWI+WE+WExpo+sabana, 
                  #sigma.formula = ~DAH+logFA+LST+logLSF+SLOPE+logTRI+TWI+WE+WExpo,
                  data = datos_training, 
                  #newdata=datos_validation, 
                  family="NBII")

modZINBI <- gamlss(INC ~ DAH+logFA+LST+logLSF+SLOPE+logTRI+TWI+WE+WExpo+sabana, 
                   #sigma.formula = ~DAH+logFA+LST+logLSF+SLOPE+logTRI+TWI+WE+WExpo,
...
modZIP_red1 <- stepGAIC(modZIP_red, parameter="sigma",
                        scope=~DAH+logFA+LST+logLSF+SLOPE+logTRI+TWI+WE+WExpo+sabana,
                        k=log(nn))
...
wp(modZINBI_red1a,ylim.all = 1.5, xlim.all = 5)
wp(modZINBI_red2a,ylim.all = 1.5, xlim.all = 10)
...
modZINBIfinal <- gamlss(modZINBI_red1a$mu.formula, 
                        sigma.formula = modZINBI_red1a$sigma.formula,
                        nu.formula = modZINBI_red1a$nu.formula,
                        data = datos_training, 
                        newdata=datos_validation, 
                        family="ZINBI")
...
save(modPO,
     ...
     modZIPfinal,
     modNBIIfinal,
     modZINBIfinal,file = "./output/models_all.RData")
...
WExpo_map <- datos_map %>% ggplot() + geom_sf(mapping = aes(col=WExpo))+
  coord_sf(crs=st_crs(3857)) 
sabana_map <- datos_map %>% ggplot() + geom_sf(mapping = aes(col=sabana))+
  coord_sf(crs=st_crs(3857)) 
pdf("covariates.pdf")
grid.arrange(DAH_map, LST_map, logLSF_map, 
             SLOPE_map, logTRI_map, TWI_map,
             WE_map,  WExpo_map,sabana_map
             , ncol=3)
dev.off()
